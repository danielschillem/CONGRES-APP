...
export function ReviewerDashboardPage() {
  ...
  const { data: assignmentsData, isLoading } = useQuery({
    ...
    queryFn: () => reviewerApi.getAssignments(statusFilter ? { status: statusFilter } : undefined),
  })

  // Load active review grid
  const { data: gridData } = useQuery({
    queryKey: ['reviewer-active-grid'],
    queryFn: async () => (await reviewerApi.getActiveReviewGrid()).data.data as ReviewGrid | null,
  })
  ...
  const submitReviewMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { scores: { criterion_id: string; score: number }[]; comment: string } }) =>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Tableau de bord Reviewer</h1>
        <p className="text-gray-500 text-sm mt-1">Gérez vos évaluations de soumissions</p>
      </div>
        ...
        <Card className="bg-primary-50 border-primary-200">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-2 text-sm text-primary-800">
              ...
              <span className="font-medium">Grille active :</span> {grid.name}
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {assignments.map((review) => {
            const state = reviewStates[review.id] ?? { scores: {}, comment: '' }
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <CardTitle className="text-base">
                        {review.soumission?.document_title ?? 'Soumission'}
                      </CardTitle>
                      <div className="flex items-center gap-2 text-sm text-gray-500">
                        <span>{review.soumission?.author_name}</span>
                        <span>·</span>
                        <span>{review.soumission?.submission_type}</span>
                        <span>·</span>
                        <span>{review.soumission?.theme}</span>
                      </div>
                    </div>
                  ...
                  {isCompleted ? (
                    <div className="space-y-3">
                      {reviewScores.length > 0 ? (
                        <div className="space-y-1">
                          {reviewScores.map((cs, i) => (
                            <div key={i} className="flex items-center gap-2 text-sm">
                              <span className="text-gray-600 w-40">{cs.criterion_name}</span>
                              <Star className="h-3.5 w-3.5 text-yellow-500 fill-yellow-500" />
                              <span className="font-semibold">{cs.score}/{cs.max_score}</span>
                            </div>
                          ))}
                          <div className="flex items-center gap-2 text-sm pt-1 border-t border-gray-100 mt-1">
                            <span className="font-medium text-gray-700">Note globale :</span>
                            <Star className="h-4 w-4 text-yellow-500 fill-yellow-500" />
                            <span className="font-semibold">{review.overall_score?.toFixed(1) ?? review.overall_score}/10</span>
                          </div>
                        </div>
                      ) : (
                        <div className="flex items-center gap-2 text-sm">
                          <Star className="h-4 w-4 text-yellow-500 fill-yellow-500" />
                          <span className="font-semibold">{review.overall_score}/10</span>
                        </div>
                      )}
                      {review.comment && (
                      ...
                      {review.status === 'assigned' ? (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            if (grid?.criteria) initReviewState(review.id)
                          }}
                        ...
                        <>
                          ...
                          {grid?.criteria && grid.criteria.length > 0 && (
                            <div className="space-y-3">
                              ...
                              {grid.criteria.map((criterion) => (
                                <div key={criterion.id} className="grid grid-cols-[1fr_120px] gap-4 items-center max-w-md">
                                  <div>
                                  ...
                                  <div className="flex items-center gap-2">
                                    <input
                                      ...
                                      value={state.scores[criterion.id] ?? 0}
                                      onChange={(e) => {
                                        const val = parseInt(e.target.value) || 0
                                        setReviewStates((prev) => ({
                                          ...prev,
                                          [review.id]: {
                                            ...prev[review.id] ?? { scores: {}, comment: '' },
                                            scores: {
                                              ...(prev[review.id]?.scores ?? {}),
                                              [criterion.id]: Math.min(val, criterion.max_score),
                                            },
                                          },
                                        }))
                                      }}
                          ...
                          {(!grid || !grid.criteria || grid.criteria.length === 0) && (
                            <div className="flex items-center gap-2">
                              <label className="text-sm font-medium">Note /10 :</label>
                              <input
                                type="number"
                                min={1}
                                max={10}
                                ...
                                onChange={(e) =>
                                  setReviewStates((prev) => ({
                                    ...prev,
                                    [review.id]: {
                                      scores: { overall: parseInt(e.target.value) || 0 },
                                      comment: prev[review.id]?.comment ?? '',
                                    },
                                  }))
                                }
                              />
                          ...
                          <div>
                            ...
                            <Textarea
                              ...
                              onChange={(e) =>
                                setReviewStates((prev) => ({
                                  ...prev,
                                  [review.id]: { ...prev[review.id] ?? { scores: {} }, comment: e.target.value },
                                }))
                              }
                              rows={4}
                            />
                          ...
                          <Button
                            ...
                            disabled={submitReviewMutation.isPending || !state.comment}
                          >
