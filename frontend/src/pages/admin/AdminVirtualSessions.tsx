...
import { CreateSessionPayload, VirtualAttendance, VirtualSession } from '@/types'
...
function SessionForm({ initial, onSubmit, onCancel }: SessionFormProps) {
  ...
  const [sessionType, setSessionType] = useState(initial?.session_type ?? 'presentation')
  const [startTime, setStartTime] = useState(initial?.start_time ?? '')
  const [endTime, setEndTime] = useState(initial?.end_time ?? '')
  const [password, setPassword] = useState(initial?.password ?? '')
  const [maxParticipants, setMaxParticipants] = useState(initial?.max_participants ?? 50)
  const [recordingEnabled, setRecordingEnabled] = useState(initial?.recording_enabled ?? false)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <Label htmlFor="recording" className="mb-0">Activer l'enregistrement</Label>
      </div>
      <DialogFooter>
...
function AttendanceDialog({ sessionId, open, onClose }: { sessionId: string; open: boolean; onClose: () => void }) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        ...
        {isLoading ? (
          ...
          <p className="text-gray-500 text-center py-8">Aucune présence enregistrée</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {attendance.map((a) => (
                <TableRow key={a.id}>
                  <TableCell>{a.user ? `${a.user.prenom} ${a.user.nom}` : a.user_id}</TableCell>
                  <TableCell>{formatDate(a.join_time)}</TableCell>
                  <TableCell>{a.leave_time ? formatDate(a.leave_time) : '-'}</TableCell>
...
export function AdminVirtualSessions() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateSessionPayload) => virtualApi.adminCreateSession(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-virtual-sessions'] })
      setShowCreate(false)
    },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: object }) => virtualApi.adminUpdateSession(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-virtual-sessions'] })
      setEditSession(null)
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-virtual-sessions'] }),
  })
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gérez les sessions de visioconférence</p>
        </div>
              ...
              <TableBody>
                {sessions.map((s) => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell>{typeLabels[s.session_type] || s.session_type}</TableCell>
                    <TableCell>
                      <Badge className={statusColors[s.status]}>{statusLabels[s.status]}</Badge>
                    </TableCell>
                    <TableCell className="text-sm">{formatDate(s.start_time)}</TableCell>
                    <TableCell className="text-sm">{formatDate(s.end_time)}</TableCell>
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {s.status === 'scheduled' && (
                            <DropdownMenuItem onClick={() => startMutation.mutate(s.id)}>
                              <Play className="h-4 w-4 mr-2" /> Démarrer
                            </DropdownMenuItem>
                          )}
                          {s.status === 'live' && (
                            <DropdownMenuItem onClick={() => endMutation.mutate(s.id)}>
                              <Square className="h-4 w-4 mr-2" /> Terminer
                            </DropdownMenuItem>
                          )}
                          <DropdownMenuItem onClick={() => setAttendanceSessionId(s.id)}>
                            <CalendarDays className="h-4 w-4 mr-2" /> Présences
                          </DropdownMenuItem>
                          <DropdownMenuItem
          ...
          {editSession && (
            <SessionForm
              initial={{
                ...
                recording_enabled: editSession.recording_enabled,
              }}
