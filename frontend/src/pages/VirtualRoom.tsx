...
function JitsiIframe({ session, userName }: { session: VirtualSession; userName: string }) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              {formatDate(session.start_time)} - {session.status === 'live' ? 'En direct' : 'Planifié'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        <Card className="w-72 shrink-0 hidden lg:block">
          <CardContent className="p-4 space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-600">{session.description || 'Aucune description'}</p>
            </div>
            ...
            {session.password && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                <p className="text-xs font-medium text-amber-800">Mot de passe requis</p>
                <p className="text-sm font-mono text-amber-700 mt-1">{session.password}</p>
              </div>
            )}
          </CardContent>
        </Card>
