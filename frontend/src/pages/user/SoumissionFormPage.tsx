...
const soumissionSchema = z.object({
  ...
  topics: z.string().min(3, 'Les sujets doivent comporter au moins 3 caractères'),
  document_title: z.string().min(5, 'Le titre doit comporter au moins 5 caractères'),
  author_name: z.string().min(3, 'Le nom de l\'auteur est requis'),
  resume: z
    .string()
    .min(50, 'Le résumé doit comporter au moins 50 caractères')
    .max(MAX_RESUME_LENGTH, `Le résumé ne peut pas dépasser ${MAX_RESUME_LENGTH} caractères`),
})
...
export function SoumissionFormPage() {
  ...
  const isEditMode = Boolean(id) && !window.location.pathname.includes('nouveau')
  ...
  const createMutation = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      setServerError(error?.response?.data?.message || 'Erreur lors de la soumission.')
    },
  })
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: unknown) => {
      const error = err as { response?: { data?: { message?: string } } }
      setServerError(error?.response?.data?.message || 'Erreur lors de la mise à jour.')
    },
  })
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    <SelectTrigger error={errors.submission_type?.message}>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="document_title">Titre du document *</Label>
              <Input
                id="document_title"
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-xs ${
                    (resumeValue?.length ?? 0) > MAX_RESUME_LENGTH
                      ? 'text-red-500'
                      : 'text-gray-400'
                  }`}
                >
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex min-h-[42px] flex-wrap gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 focus-within:ring-2 focus-within:ring-primary-500 focus-within:ring-offset-2">
                ...
                <input
                  ...
                  placeholder={keywords.length === 0 ? 'Tapez un mot-clé et appuyez sur Entrée...' : ''}
                  className="flex-1 min-w-[150px] border-none bg-transparent text-sm outline-none placeholder:text-gray-400"
                />
              </div>
              <p className="text-xs text-gray-400">Appuyez sur Entrée ou virgule pour ajouter un mot-clé</p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            >
              ...
              {file ? (
                ...
                <div className="text-center">
                  <p className="text-sm font-medium text-gray-700">Fichier actuel</p>
                  <p className="text-xs text-gray-400 mt-1">
                    {existingData.file_path.split('/').pop()}
                  </p>
                  <p className="text-xs text-primary-500 mt-1">Cliquez pour remplacer</p>
                </div>
              ) : (
                <div className="text-center">
            ...
            {!editId && (
              <p className="mt-1.5 text-xs text-gray-400">* Le fichier PDF est obligatoire pour une nouvelle soumission.</p>
            )}
          </CardContent>
        </Card>
